...
class PumpScannerService:
    def __init__(self, config: DetectorConfig, alert_config: AlertConfig) -> None:
        ...
        self.market_caps_retry_after: datetime | None = None
        self.market_caps_failures = 0
        self.last_market_caps_error: str | None = None
    ...
    async def _refresh_market_caps_if_needed(self) -> None:
        if self.market_caps_retry_after and datetime.utcnow() < self.market_caps_retry_after:
            return
        ...
        if needs_refresh:
            ...
            try:
                ...
                self.market_caps_failures = 0
                self.market_caps_retry_after = None
                self.last_market_caps_error = None
                logger.info("Market cap cache refreshed with %d symbols", len(self.market_caps))
            except Exception as exc:  # noqa: BLE001
                self.market_caps_failures += 1
                delay_seconds = min(300, 15 * (2 ** min(self.market_caps_failures, 4)))
                self.market_caps_retry_after = datetime.utcnow() + timedelta(seconds=delay_seconds)
                self.last_market_caps_error = str(exc)
                logger.warning(
                    "Market cap refresh failed (%s). Retry in %ss",
                    exc,
                    delay_seconds,
                )
    ...
    def health(self) -> dict[str, object]:
        return {
            ...
            "market_caps_retry_after": self.market_caps_retry_after.isoformat()
            if self.market_caps_retry_after
            else None,
            "market_caps_failures": self.market_caps_failures,
            "last_market_caps_error": self.last_market_caps_error,
